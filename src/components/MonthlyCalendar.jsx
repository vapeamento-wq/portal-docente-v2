        ...
        >
            ...
            <div className="grid grid-cols-7 gap-2 sm:gap-4 auto-rows-fr">
                {calendarCells.map(cell => (
                    <div
                        key={cell.id}
                        className={`min-h-[100px] sm:min-h-[120px] p-2 rounded-xl border transition-all duration-300 relative flex flex-col
                            ${!cell.isCurrentMonth ? 'bg-transparent border-transparent opacity-0' :
                                cell.isToday ? 'bg-blue-50/50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' :
                                    'bg-gray-50 dark:bg-slate-800/50 border-gray-200 dark:border-slate-700 hover:border-[#db9b32] dark:hover:border-yellow-500'
                            }`}
                    >
                        {cell.isCurrentMonth && (
                            <>
                                <div className={`text-right font-bold text-lg mb-2 ${cell.isToday ? 'text-[#2D8CFF]' : 'text-gray-400 dark:text-gray-500'}`}>
                                    {cell.dayNumber}
                                </div>
                                <div className="flex flex-col gap-2 flex-1 overflow-y-auto custom-scrollbar">
                                    {cell.events.map((ev, eIdx) => (
                                        <div
                                            key={eIdx}
                                            className={`text-xs p-2 rounded-lg border flex flex-col gap-1 shadow-sm
                                                ${ev.status === 'past' ? 'bg-gray-100 dark:bg-slate-700 border-gray-300 dark:border-slate-600 opacity-60' :
                                                    ev.status === 'present' ? 'bg-[#25D366] text-white border-[#25D366]' :
                                                        'bg-[#003366] text-white border-[#003366] dark:bg-blue-600 dark:border-blue-500'
                                                }`}
                                        >
                                            <div className="font-bold flex justify-between items-center">
                                                <span>Sem {ev.num}</span>
                                                <span className="opacity-80 text-[0.65rem]">{ev.hora !== '00 a 00' ? ev.hora : ''}</span>
                                            </div>

                                            <div className="font-medium truncate" title={ev.displayTexto}>
                                                {ev.tipo === 'INDEPENDIENTE' ? '🏠 ' : '🏫 '}
                                                {ev.displayTexto}
                                            </div>

                                            {ev.ubicacion && ev.tipo === 'PRESENCIAL' && (
                                                <div className="text-[0.65rem] opacity-90 mt-0.5 font-semibold">📍 {ev.ubicacion}</div>
                                            )}
                                            ...
                                            {ev.zoomLink && (
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="mt-1 flex items-center justify-center gap-1 bg-white/20 hover:bg-white/30 text-white rounded py-1 px-1 transition-colors text-[0.65rem] font-bold decoration-0"
                                                    onClick={() => registrarLog(docenteId, `🎥 Zoom Sem ${ev.num}`)}
                                                >
                                                    🎥 {ev.zoomId ? `ID: ${ev.zoomId}` : 'Enlace Zoom'}
                                                </a>
                                            )}
                                        </div>
                                    ))}
